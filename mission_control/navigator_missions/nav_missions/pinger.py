...
class PingerMission:
        ...
        if self.USE_CLOSE_POINTS:
            ...
            if self.negate:
                self.observation_points = (
                    np.append((self.gate_poses[0] - self.OBSERVE_DISTANCE_METERS * self.g_perp), 0), np.append(
                        (self.gate_poses[2] - self.OBSERVE_DISTANCE_METERS * self.g_perp), 0))
            else:
                self.observation_points = (
                    np.append((self.gate_poses[0] + self.OBSERVE_DISTANCE_METERS * self.g_perp), 0), np.append(
                        (self.gate_poses[2] + self.OBSERVE_DISTANCE_METERS * self.g_perp), 0))
    ...
    def get_colored_buoys(self):
        ...
        if self.negate:
            estimated_circle_buoy = self.gate_poses[1] + (
                self.g_perp * 30.0)  # should check on correct side (negate bool)
        else:
            estimated_circle_buoy = self.gate_poses[1] - (
                self.g_perp * 30.0)  # should check on correct side (negate bool)
        ...
        if totems.found:
            ...
            sorted_circle = sorted(totems.objects, key=lambda t: abs(
                np.linalg.norm(estimated_circle_buoy - mil_tools.rosmsg_to_numpy(t.position)[:2])))
            self.new_marker(position=mil_tools.rosmsg_to_numpy(
                sorted_circle[0].position), color=(1, 1, 1), time=cur_time)
            if np.linalg.norm(mil_tools.rosmsg_to_numpy(
                    sorted_circle[0].position)[:2] - estimated_circle_buoy) < self.MAX_CIRCLE_BUOY_ERROR:
    ...
    def circle_buoy(self):
        if self.circle_totem is not None:
            # Circle around totem
            yield self.navigator.move.look_at(
                self.circle_totem).set_position(self.circle_totem).backward(8).yaw_left(90, unit='deg').go()

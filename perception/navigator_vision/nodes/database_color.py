...
class Observation(object):
    ...
    def compute_confidence(self, (value_w, dist_w, q_diff_w), **kwargs):
        ...
        v_u = kwargs.get('v_u', 200) 
        v_sig = kwargs.get('v_sig', 70)
        dist_sig = kwargs.get('dist_sig', 20)
...
class Colorama(object):
    def __init__(self):
        ...
        # Confidence weights
        self.v_factor = 0.3         # Favor value values close to the nominal value
        self.v_u = 200              # Mean of norm for variance error
        self.v_sig = 70             # Sigma of norm for variance error
        self.dist_factor = 1        # Favor being closer to the totem
        self.dist_sig = 20          # Sigma of distance (m)
        self.q_factor = 0.0         # Favor not looking into the sun
    ...
    def empty_template(self):
        """
        Returns a dictionary mapping all colors to ColorObservation objects
        """
        color_pair_template = {}
        ColorObservation.history_length = self.history_length
        for color in self.color_map:
            color_pair_template[color] = ColorObservation()
        return color_pair_template
    ...
    def do_observe(self, *args):
        ...
        if resp.found:
            ...
            time_of_marker = resp.objects[0].header.stamp
            image_holder = self.image_history.get_around_time(time_of_marker)
            if not image_holder.contains_valid_image:
                fprint("No valid image found for t={} ({})".format(time_of_marker, rospy.Time.now()), msg_color='red')
                return
        ...
        if resp.found:
            ...
            for obj in resp.objects:
                if len(obj.points) > 0 and obj.name == "totem":
                    ...
                    self.remove_below = np.mean(points_np[:, 2]) - np.std(points_np[:, 2]) / 2.0
                    fprint("Removing below {}".format(self.remove_below))
                    points_np = points_np[points_np[:, 2] > self.remove_below]
                    if len(points_np) == 0:
                        fprint("No valid points")
                        continue
                    ...
                    if color_info is not None:
                        ...
                        c = self.database_color_map[color]
                        if obj.id not in self.colored:
                            self.colored[obj.id] = []

                        self.colored[obj.id].append({'color':color, 'err':error})
                        self.colored[obj.id] = self.colored[obj.id][:self.only_keep]
        
                        if self.valid_color(obj.id):
                            fprint("COLOR IS VALID", msg_color='green')
                            self.make_request(cmd='{name}={bgr[0]},{bgr[1]},{bgr[2]},{_id}'.format(name=obj.name,_id=    obj.id, bgr=bgr))
                        else:
                            fprint("COLOR IS NOT VALID", msg_color='red')
                            self.make_request(cmd='{name}=0,0,0,{_id}'.format(name=obj.name,_id=obj.id, bgr=bgr))
                    elif self.valid_color(obj.id):
                        self.make_request(cmd='{name}=0,0,0,{_id}'.format(name=obj.name,_id=obj.id, bgr=bgr))

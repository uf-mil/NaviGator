...
class MRAC_Controller:
    ...
    def __init__(self):
        ...
        self.thruster_positions = np.array(
            [[-1.9000, 1.0000, -0.0123], [-1.9000, -1.0000, -0.0123], [1.6000, 0.6000, -0.0123],
                [1.6000, -0.6000, -0.0123]])  # back-left, back-right, front-left front-right, m
        self.thruster_directions = np.array(
            [[0.7071, 0.7071, 0.0000], [0.7071, -0.7071, 0.0000], [0.7071, -0.7071, 0.0000],
        ...
        self.D_body_positive = abs(
            np.array([self.Fx_max_body[0], self.Fy_max_body[1], self.Mz_max_body[5]])) / self.vel_max_body_positive**2
        self.D_body_negative = abs(
            np.array([self.Fx_max_body[0], self.Fy_max_body[1], self.Mz_max_body[5]])) / self.vel_max_body_negative**2
    ...
    def set_traj(self, msg):
        ...
        self.q_ref = np.array(
            [msg.posetwist.pose.orientation.x, msg.posetwist.pose.orientation.y,
                msg.posetwist.pose.orientation.z, msg.posetwist.pose.orientation.w])

        R = trns.quaternion_matrix(self.q_ref)[:3, :3]

        self.v_ref = R.dot(
            np.array([msg.posetwist.twist.linear.x, msg.posetwist.twist.linear.y, msg.posetwist.twist.linear.z]))[:2]
        self.w_ref = R.dot(
            np.array([msg.posetwist.twist.angular.x, msg.posetwist.twist.angular.y, msg.posetwist.twist.angular.z]))[2]

        self.a_ref = R.dot(
            np.array(
                [msg.posetwist.acceleration.linear.x, msg.posetwist.acceleration.linear.y,
                    msg.posetwist.acceleration.linear.z]))[:2]
        self.aa_ref = R.dot(
            np.array(
                [msg.posetwist.acceleration.angular.x, msg.posetwist.acceleration.angular.y,
    ...
    def get_command(self, msg):
        ...
        inertial_feedforward = np.concatenate((self.a_ref, [self.aa_ref])) * [
            self.mass_ref, self.mass_ref, self.inertia_ref]

        # Compute the "learning" matrix
        drag_regressor = np.array(
            [[self.lin_vel[0] * np.cos(
                y)**2 + self.lin_vel[1] * np.sin(
                    y) * np.cos(
                        y), self.lin_vel[0] / 2 - (
                            self.lin_vel[0] * np.cos(
                                2 * y)) / 2 - (
                                    self.lin_vel[1] * np.sin(
                                        2 * y)) / 2, -self.ang_vel * np.sin(
                                            y), -self.ang_vel * np.cos(y), 0],
             [self.lin_vel[1] / 2 - (self.lin_vel[1] * np.cos(2 * y)) / 2 + (
                 self.lin_vel[0] * np.sin(2 * y)) / 2, self.lin_vel[1] * np.cos(y)**2 - self.lin_vel[
              0] * np.cos(
              y) * np.sin(
              y), self.ang_vel * np.cos(
              y), -self.ang_vel * np.sin(
              y), 0],
             [0, 0, self.lin_vel[1] * np.cos(y) - self.lin_vel[0] * np.sin(
                 y), - self.lin_vel[0] * np.cos(y) - self.lin_vel[1] * np.sin(y), self.ang_vel]])
        ...
        if self.only_PD:
            ...
            if self.learn and (npl.norm(p_err) < self.learning_radius):
                self.dist_est = np.clip(
                    self.dist_est + (self.ki * err * self.timestep), -self.dist_limit, self.dist_limit)
        ...
        self.pose_ref_pub.publish(PoseStamped(header=Header(frame_id='/enu', stamp=msg.header.stamp), pose=Pose(
            position=Point(self.p_ref[0], self.p_ref[1], 0), orientation=Quaternion(*self.q_ref))))

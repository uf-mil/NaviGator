...
public:
  ...
  {
    std::vector<std::string> camera_names = { "/stereo/right/image_rect_color", "/camera/front/right/"
                                                                                "image_rect_color" };
    nh_.param<std::vector<std::string>>("/pc_colorizer/cameras", camera_names, camera_names);

    // what topic to subscribe to
    std::string pointcloud_subscriber = "/velodyne_points";
    nh_.param<std::string>("/pc_colorizer/pointcloud_subscriber", pointcloud_subscriber, pointcloud_subscriber);

    // what should the publisher topic name be
    std::string pointcloud_advertiser = "/velodyne_points/colored";
    nh_.param<std::string>("/pc_colorizer/pointcloud_advertiser", pointcloud_advertiser, pointcloud_advertiser);
    ...
    point_cloud_sub_ =
        nh_.subscribe<pcl::PointCloud<pcl::PointXYZRGB>>("/velodyne_points", 1, &Colorizer::cb_velodyne, this);

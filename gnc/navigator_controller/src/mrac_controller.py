...
class MRAC_Controller:
    ...
    def __init__(self):
        ...
        self.kp_body = np.diag(rospy.get_param('~p_gains_body'))
        # Derivative gains, body frame
        self.kd_body = np.diag(rospy.get_param('~d_gains_body'))
        # Disturbance adaptation rates, world frame
        self.ki = np.array(rospy.get_param('~i_gains_body'))
        # Drag adaptation rates, world frame
        self.dist_est = np.array(rospy.get_param('~disturbance_estimate'))
        # Initial drag estimate
        self.drag_est = np.array(rospy.get_param('~drag_estimate'))  # [d1 d2 Lc1 Lc2 Lr]
        # Limits on disturbance estimate
        self.dist_limit = np.array(rospy.get_param('~disturbance_limits'))
        # Limits on drag estimate effort
        self.drag_limit = np.array(rospy.get_param('~drag_limit'))
        # User-imposed speed limit
        self.vel_max_body_positive = np.array(rospy.get_param('~max_positive_velocity'))  # [m/s, m/s, rad/s]
        self.vel_max_body_negative = np.array(rospy.get_param('~max_negative_velocity'))  # [m/s, m/s, rad/s]
        # "Smart heading" threshold
        self.heading_threshold = rospy.get_param('~heading_threshold')  # m
        # Only use PD controller
        self.only_PD = rospy.get_param('~PD_only')
        ...
        self.use_external_tgen = rospy.get_param('~external_tgen')
        ...
        self.mass_ref = rospy.get_param('~mass_reference')  # kg, determined to be larger than boat in practice due to water mass
        self.inertia_ref = rospy.get_param('~inertia_reference')  # kg*m^2, determined to be larger than boat in practice due to water mass
        self.thrust_max = rospy.get_param('~max_thrust')  # N

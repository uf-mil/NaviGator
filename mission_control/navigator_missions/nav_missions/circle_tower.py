...
def main(navigator, **kwargs):
    ...
    for color in colors:
        ...
        if target is None or _dist_from_bf(
                mil_tools.rosmsg_to_numpy(target.position)) > (BF_WIDTH / 2 + BF_EST_COFIDENCE):
            # Need to do something
            fprint(
                "No suitable totems found, going to circle any nearby totems.", msg_color='red', title="CIRCLE_TOTEM")

...
# ACTION
    ...
    def move_cb(self, msg):
        ...
        # Standard driving
            ...
            if abs(
                self.angle_diff(
                    h_goal, self.state[2])) > params.pointshoot_tol and npl.norm(
            ...
            while np.all(np.less_equal(push, len(occ_img_dial))):
                ...
                for (row, col) in bpts:
                    ...
                    if test_flood[ss_seed[1], ss_seed[0]] == 69:
                        gs[:2] = (np.add([col, row], [last_offsets[0], last_offsets[1]]).astype(
                            np.float64) / self.ogrid_cpm) + self.ogrid_origin
                        found_entry = True
                        break
        ...
        if next_runtime is not None and last_update_time is not None:
            ...
            if self.move_type == 'spiral':
                self.move_server.publish_feedback(MoveFeedback('boat',
                                                               0,
                                                               self.tracking,
                                                               self.erf(self.goal, self.get_ref(
                                                                   self.rostime() - last_update_time))[:3],
        ...
        if self.get_ref is not None and last_update_time is not None:
            if np.all(
                np.abs(
                    self.erf(
                        self.get_ref(
                            self.rostime() - last_update_time), self.state)) < 2 * np.array(params.real_tol)):
                self.tracking = True
            else:
                self.tracking = False
...
# CONVERTERS
    ...
    def pack_posestamped(self, state, stamp):
        ...
        msg.pose.orientation.w = trns.quaternion_from_euler(
            0, 0, state[2])
        return msg
    ...
    def pack_odom(self, state, stamp):
        ...
        msg.pose.pose.orientation.w = trns.quaternion_from_euler(
            0, 0, state[2])

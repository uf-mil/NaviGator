...
class DetectDeliverMission:
    ...
    def set_shape_and_color(self):
        ...
        fprint("Color={} Shape={} Target={}".format(
            self.Color, self.Shape, target), title="DETECT DELIVER", msg_color='green')
            ...
            for shape in shapes.shapes.list:
                ...
                if normal_res.success:
                    ...
                    fprint("Normal not found Error={}".format(
                        normal_res.error), title="DETECT DELIVER", msg_color='red')
        else:
    ...
    def circle_search(self):
        ...
        yield self.navigator.move.look_at(platform_np).set_position(platform_np).backward(
            self.circle_radius).yaw_left(90, unit='deg').go(move_type="drive")

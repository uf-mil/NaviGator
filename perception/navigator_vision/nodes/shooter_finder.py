    ...
    for i in xrange(4):
        ...
        yield (p, normal), numpy.array([
            [1, 0, normal[0] / 2 if i % 2 == 0 else 0, normal[0] / 2 if i %
                2 == 1 else 0, (rect[2] if i % 2 == 0 else rect[3]) / 2 * d_normal_over_d_angle[0]],
            [0, 1, normal[1] / 2 if i % 2 == 0 else 0, normal[1] / 2 if i %
                2 == 1 else 0, (rect[2] if i % 2 == 0 else rect[3]) / 2 * d_normal_over_d_angle[1]],
...
def dist_from_line(p, line, J):
    return (p - line[0]).dot(line[1]), numpy.array(
        [-line[1][0], -line[1][1], p[0] - line[0][0], p[1] - line[0][1]]).dot(J)


def dist_from_rect(p, rect):
    return min([dist_from_line(p, line, J) for line, J in rect_lines(rect)], key=lambda dist_J: abs(dist_J[0]))
...
def main():
    try:
        ...
        while True:
            ...
            good_points = numpy.array([(x[0], x[1])
            for x in gen if x[2] > Z_MIN and math.hypot(x[0] - center[0], x[1] - center[1]) < RADIUS])
            print 'end'

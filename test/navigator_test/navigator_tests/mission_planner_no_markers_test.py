...
class MissionPlannerTest(TestUnit):
    ...
    def run_tests(self):
        ...
        yield self._run_mission(
            base_file + "/missing_object.yaml", self.pub_missing_objects,
            self.service_missing, "Testing what "
            "happens when a mission loses an object dependency in the middle of the run. "

...
import tf
from videotest import image_converter
import keras
...
class ObjectImageClassifier(object):
    ...
    def __init__(self, class_names, model, input_shape):
        ...
        self.camera_tfs = ["/front_left_cam"]
        ...
        self.ic = image_converter(
            self.class_names, self.model, self.input_shape)
        ...
        self.DBQuery_proxy = rospy.ServiceProxy(
            '/database/requests', ObjectDBQuery)

        while not rospy.is_shutdown():
            try:
                [tf_listener.waitForTransform("enu", t, rospy.Time(
                ), rospy.Duration(5)) for t in self.camera_tfs]
                # get a list for all points and all labels for each camera
                info = [self.get_label_and_points(
                    c.frame) for c in self.cameras]

                # get pose for all the cameras
                enu_cam_tf = [tf_listener.lookupTransform("enu", self.camera_tfs[i], self.cameras[
                                                          i].sub.last_image_time) for i in range(len(self.cameras))]

                cam_p, cam_q = zip(*enu_cam_tf)
                cam_p = [np.array(p) for p in cam_p]
                cam_r = [tf.transformations.quaternion_matrix(
                    q)[:3, :3] for q in cam_q]
                all_objects = None
                try:
                    all_objects = self.DBQuery_proxy(
                        name="all", cmd="").objects
                    # print("called")
                    # print(all_objects)
                ...
                for i in range(len(self.cameras)):
                    ...
                    for j in range(len(points)):
                        ray = self.get_ray_in_enu(points[j], self.cameras[
                                                  i].img_geom, cam_r[i])
                        if ray is None:
                            continue
                        # print("hey")
                        o = self.find_closest_object_given_ray(
                            all_objects, ray, cam_p[i])
                        if o is not None:
    ...
    def get_label_and_points(self, img):
    	# print(img.shape)
    	# assert(False)
        points, labels = self.ic.get_predictions(img)
    ...
    def find_closest_object_given_ray(self, objects, ray, ray_base, tol=3):
        for o in objects:
            distance = np.linalg.norm(
                np.cross(ray, mil_tools.rosmsg_to_numpy(o.pose.position) - ray_base))
            # print (distance)
            if distance > tol:
                continue
            else:
    ...
    def set_label_for_object(self, label, o):
        try:
            all_objects = self.DBQuery_proxy(
                name="", cmd="{}={}".format(o.id, label)).objects
...
if __name__ == "__main__":
    ...
    class_names = ["Background", "Green Totem", "Blue Totem",
    ...
    model.load_weights("weights_latest_iter.hdf5")

    big_boi = ObjectImageClassifier(class_names, model, input_shape)

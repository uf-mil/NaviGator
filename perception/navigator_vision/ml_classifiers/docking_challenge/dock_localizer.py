...
sys.path.append(
    rospack.get_path('navigator_vision') + '/ml_classifiers/docking_challenge/utils')
...
class classifier(object):
    ...
    def __init__(self):
        ...
        self.num_objects_detect = rospy.get_param('~objects_detected', 10)
        # Mininum confidence score for the detections
        self.score_thresh = rospy.get_param('~score_thresh', 0.0000001)
        # If we want debug images published or not.
        self.debug = rospy.get_param('~debug', True)
        # Camera image width and height
        self.im_width = rospy.get_param('~im_width', 1920)
        self.im_height = rospy.get_param('~im_height', 1080)
        # Current time
